...
void RDA5807M::setFrequency(RADIO_FREQ newF) {
    ...
    registers[RADIO_REG_CTRL] |= RADIO_REG_CTRL_OUTPUT | RADIO_REG_CTRL_UNMUTE | RADIO_REG_CTRL_RDS |
                                 RADIO_REG_CTRL_ENABLE; //  | RADIO_REG_CTRL_NEW
    _saveRegister(RADIO_REG_CTRL);
...
void RDA5807M::checkRDS() {
    ...
    if (_sendRDS) {
        ...
        if (registers[RADIO_REG_RA] & RADIO_REG_RA_RDS) {
            ...
            Wire.requestFrom(I2C_INDX, 8,
                             1);                  // Retransmit device address with READ, followed by 8 bytes
            newData = _read16();
            ...
            if (result) {
                ...
                // send to RDS decoder
                _sendRDS(registers[RADIO_REG_RDSA], registers[RADIO_REG_RDSB], registers[RADIO_REG_RDSC],
                         registers[RADIO_REG_RDSD]);
            } // if
        } // if
    }
}
